	...
	public class GlowLayer : PlayerDrawLayer
	{
		...
		protected override void Draw(ref PlayerDrawSet drawInfo) {
			...
			if (parryPlayer.SwordParry) {
				glowPosition = 3 * (player.GetFrontHandPosition(player.compositeFrontArm.stretch, player.compositeFrontArm.rotation - 1 * player.direction) - player.MountedCenter) + NullandVoidUtils.ScreenCenter();
			}
			else {
				glowPosition = 2 * (player.GetBackHandPosition(player.compositeBackArm.stretch, player.compositeBackArm.rotation - 1 * player.direction) - player.MountedCenter) + NullandVoidUtils.ScreenCenter();
			}
